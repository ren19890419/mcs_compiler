...
void CodeEdit::SetDefaultLexer()
{
    ...
    // Set the autocomplete icons. 设置各种类别的图标 函数，模块变量图标
	...
	wxBitmap variableIcon(variableicon, wxBITMAP_TYPE_XPM);
	variableIcon.SetMask(new wxMask(variableicon, maskColor));

	RegisterImage(AutoCompleteManager::Type_Variable, variableIcon);
...
bool CodeEdit::GetTokenFromPosition(int position, const wxString& joiners, wxString& token)
{
    ...
    if (position != -1)
    {
		...
		//text = text.FromUTF8(text);
		if (text[seek] < -1 || text[seek]>255)
			return false;
        ...
        int start = seek;
		if (start > 0 && (text[start - 1] < -1 || text[start - 1]>255))
			return false;
		...
		if(end + 1 < text.Length() && (text[end + 1] < -1 || text[end + 1]>255))
			return false;
...
void CodeEdit::StartAutoCompletion(const wxString& token)
{
    ...
    wxString prefix,module_name,func_or_menber_name;
    bool member = false;

	if (GetLexer() == wxSCI_LEX_LUA)
	{
		//找到第一个.或:的位置 取到函数名称
		int cln_index = token.Find(':', true);
		int dot_index = token.Find('.', true);
		if ((cln_index != wxNOT_FOUND) || (dot_index != wxNOT_FOUND))
		{
			if (token.Length() > std::max(cln_index, dot_index) + 1)
				func_or_menber_name = token.Right(token.Length() - std::max(cln_index, dot_index) - 1);
			else
			{
				func_or_menber_name = "";
				member = true;
			}
			wxString temp_module_name;
			if (std::max(cln_index, dot_index) - 1 > 0)
			{
				temp_module_name = token.Left(std::max(cln_index, dot_index));
				int modu_cln_index = temp_module_name.Find(':', true);
				int modu_dot_index = temp_module_name.Find('.', true);
				if ((modu_cln_index != wxNOT_FOUND) || (modu_dot_index != wxNOT_FOUND))
					module_name = temp_module_name.Right(temp_module_name.Length() - std::max(modu_cln_index, modu_dot_index) - 1);
				else
		...
		if(func_or_menber_name != "")
			prefix = func_or_menber_name;
		else
		{
			if (module_name != "")
				prefix = module_name;
    ...
    m_autoCompleteManager->GetMatchingItems(prefix, items, module_name, func_or_menber_name);
